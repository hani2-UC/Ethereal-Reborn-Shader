...
void main() {
    ...
    vec3 directColor = mix(vec3(1.00, 0.86, 0.65), vec3(0.46, 0.61, 1.00), night);
    vec3 ambientTint = mix(vec3(0.17, 0.21, 0.29), vec3(0.27, 0.33, 0.47), night);
    vec3 lighting = vanillaLight * 0.66 + ambientTint * 0.21;
    lighting += directColor * normalLight * shadow * (0.58 - rainStrength * 0.24);
